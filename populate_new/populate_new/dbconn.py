...
def create_mongo_connections():
    ...
    source_client = MongoClient(host=host, port=port, username=username, password=password, authSource=auth_source, document_class=RawBSONDocument)
    source_db = source_client[database]
    print("connecting to target database")
    host, port, database, username, password, auth_source = read_env_variables("TARGET")
    target_client = MongoClient(host=host, port=port, username=username, password=password, authSource=auth_source, document_class=RawBSONDocument)
